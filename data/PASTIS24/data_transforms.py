...
def PASTIS_segmentation_transform(model_config, is_training):
    ...
    inputs_backward = get_params_values(model_config, 'inputs_backward', True)
    transform_list = []
    ...
    transform_list.append(CutOrPad(max_seq_len=max_seq_len, random_sample=False, from_start=False))  # pad with zeros to maximum sequence length
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        tensor_sample['labels'] = torch.tensor(sample['labels'].astype(np.int64))
...
class Normalize(object):
    ...
    def __call__(self, sample):
        if self.compute_stats:
            # 统计量累积模式
            img = sample['inputs']  # 输入形状应为 [T, C, H, W]
            if self.total_sum is None:
                # 初始化统计量存储
                self.total_sum = torch.zeros(img.size(1), dtype=torch.float64)
                self.total_sq_sum = torch.zeros(img.size(1), dtype=torch.float64)

            # 按通道累加像素值总和和平方和
            self.total_sum += img.sum(dim=(0, 2, 3)).double()  # 形状 [C]
            self.total_sq_sum += (img ** 2).sum(dim=(0, 2, 3)).double()  # 形状 [C]
            self.num_pixels += img.shape[0] * img.shape[2] * img.shape[3]  # T*H*W
        else:
            # 直接使用类变量中的统计量
            if Normalize.computed_mean is None or Normalize.computed_std is None:
                raise ValueError("均值和标准差未计算，请先运行统计量计算阶段")
            sample['inputs'] = (sample['inputs'] - Normalize.computed_mean) / Normalize.computed_std
    ...
    def compute_mean_std(self):
        ...
        if self.num_pixels == 0:
            raise RuntimeError("未累积任何数据，无法计算统计量")

        mean = self.total_sum / self.num_pixels
        std = torch.sqrt(self.total_sq_sum / self.num_pixels - mean ** 2)

        # 转换为Float32并调整形状以匹配输入 [1, C, 1, 1]
        self.mean = mean.float().view(1, -1, 1, 1)
        self.std = std.float().view(1, -1, 1, 1)

        # 重置统计量
        self.total_sum = None
        self.total_sq_sum = None
        self.num_pixels = 0
        Normalize.computed_mean = self.mean
        Normalize.computed_std = self.std
