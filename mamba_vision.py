...
class SimpleMambaSSM(nn.Module):
    """
    纯 PyTorch 实现的 Mamba SSM Block，支持长序列建模。
    适合作为 TSViT 或 MambaVisionMixer 的时序模块。
    """
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        # SSM核心循环
        # 逐通道递归，等价于论文的状态空间 ODE 离散化
        # 实际可替换为更高效的 scan/cumsum 或 FFT 卷积
        y = []
        for i in range(self.d_inner):
            ...
            o = []
            for t in range(L):
                # 状态递推：h_t+1 = A * h_t + B * u_t
                h = self.A[i] * h + self.B[i] * u[:, t:t + 1]
                # 输出：C * h_t + D * u_t
                out_t = (self.C[i] * h).sum(dim=1) + self.D[i] * u[:, t]
                o.append(out_t)
            o = torch.stack(o, dim=1)  # (B, L)
            y.append(o)
        ...
        # 计算特征选择权重 (特征维度的重要性)
        feature_importance = torch.mean(torch.abs(y), dim=[0, 1])  # [D]

        # 计算时间步关注度 (每个时间步的重要性)
        timestep_importance = torch.mean(torch.abs(y), dim=[0, 2])  # [L]

        # 存储分析结果
        self.feature_importance = feature_importance.detach().cpu().numpy()
        self.timestep_importance = timestep_importance.detach().cpu().numpy()

        return y

    def get_feature_importance(self):
        return self.feature_importance, self.timestep_importance
...
class MambaVisionMixer(nn.Module):
    ...
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2, **kwargs):
        ...
        # 添加位置重要性属性
        self.position_importance = None
        self.feature_importance = None
        self.timestep_importance = None
    ...
    def forward(self, hidden_states):
        # hidden_states: (B, L, D)
        # print(f"[MambaVisionMixer] 输入形状: {hidden_states.shape}")
        ...
        # 计算位置重要性 (沿特征维度平均)
        position_importance = torch.mean(torch.abs(x), dim=2)  # [B, L]
        self.position_importance = position_importance.mean(dim=0).detach().cpu().numpy()  # [L]

        # 计算特征重要性 (沿时间维度平均)
        self.feature_importance = torch.mean(torch.abs(x), dim=[0, 1]).detach().cpu().numpy()  # [D]

        # 计算时间步重要性 (沿特征维度平均)
        self.timestep_importance = torch.mean(torch.abs(x), dim=2).mean(dim=0).detach().cpu().numpy()  # [L]
        # print(f"[MambaVisionMixer] 位置关注度形状: {self.position_importance.shape}")

        return x

    def get_position_importance(self):
        """获取位置重要性"""
        return self.position_importance
